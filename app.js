...
const wordleRouter = require('./routes/wordleRouter.js');
...
expressApp.initializeWebSocket = (server) => {
...
expressApp.onStartup = async () => {
	...
	try {
		...
		debug.log('Word lists initialized successfully.');
	} catch (err) {
		debugErrorLog('Failed to initialize word lists:', err);
		process.exit(1);
	}
...
expressApp.use(config.baseUrl, wordleRouter);
